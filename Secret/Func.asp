...
'=============================
'获取评分状态
Function intGetGradeState(Id,SType)
	set GGS=server.CreateObject("adodb.recordset")
	if SType=0 then
		Sql="select GradeState from Examine where ExamineId="&Id&""
	elseif SType=1 then
		Sql="select GradeState from Examine E "&_
			"left join ExamineItem EI on EI.ExamineId=E.ExamineId "&_
			"where EI.ExaItemId="&Id&""
	end if
	GGS.open Sql,G_DBConn,1,1,1
	if not GGS.eof then
		intGetGradeState=GGS("GradeState")	
	end if
	GGS.Close
end Function
'=============================
...
Function TotalScore(ExamineId)
	...
	if intGetGradeState(ExamineId,0)="4" then
		StrSql="select Sum(isnull(convert(decimal(10,2),round(ResultMarks,2)),0)) as Marks "&_
		"from ExamineItem where ExamineId="&ExamineId&""
	else
		StrSql="select EI.ExamineId, Sum(case EI.MarksType when 0 then isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) else isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) end) Marks  "&_
		...
		"group by EI.ExamineId "
	end if
	TotSco.open StrSql,G_DBConn,1,1,1
...
Function FactorScore(ExaItemId)
	...
	if intGetGradeState(ExaItemId,1)="4" then
		StrSql="select isnull(convert(decimal(10,2),round(ResultMarks,2)),0) Marks "&_
		"from ExamineItem where ExaItemId="&ExaItemId&""
	else
		StrSql="select EI.ExamineId,case EI.MarksType when 0 then isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) else isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) end Marks "&_
		"from ExamineItem EI "&_
		"left join (select ExaItemId,sum(isnull(convert(decimal(10,2),round(Weighing*Marks/100,2)),0)) as Marks "&_
		"from ExaEmpTab group by ExaItemId) EET on EET.ExaItemId=EI.ExaItemId where EI.ExaItemId="&ExaItemId&""
	end if
	FactSco.open StrSql,G_DBConn,1,1,1
...
Function ExaTotalScore(ExamineId,EmpCode)
	...
	if intGetGradeState(ExamineId,0)="4" then
		StrSql="select EI.ExamineId,EET.ExaEmpCode,"&_
		"sum(isnull(convert(decimal(10,2),round(EET.ResultMarks*EI.Weighing/100,2)),0)) Marks "&_
		"from ExamineItem EI "&_
		"left join ExaEmpTab EET on EET.ExaItemId=EI.ExaItemId "&_
		"where EI.ExamineId="&ExamineId&" and EET.ExaEmpCode='"&EmpCode&"' group by EI.ExamineId,EET.ExaEmpCode "
	else
		StrSql="select EI.ExamineId,EET.ExaEmpCode, "&_
		"sum(case EI.MarksType when 0 then isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) else isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) end) Marks "&_
		"from ExamineItem EI "&_
	...
	ExaTotSco.open StrSql,G_DBConn,1,1,1
...
Function ExaFactorScore(ExaItemId,EmpCode)
	...
	if intGetGradeState(ExaItemId,1)="4" then
		StrSql="select EI.ExamineId,EI.ExaItemId,EET.ExaEmpCode, "&_
		"isnull(convert(decimal(10,2),round(EET.ResultMarks*EI.Weighing/100,2)),0) Marks "&_
		"from ExamineItem EI "&_
		"left join ExaEmpTab EET on EET.ExaItemId=EI.ExaItemId "&_
		"where EI.ExaItemId="&ExaItemId&" and EET.ExaEmpCode='"&EmpCode&"'"
	else
		StrSql="select EI.ExamineId,EI.ExaItemId,EET.ExaEmpCode, "&_
		"case EI.MarksType when 0 then isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) else isnull(convert(decimal(10,2),round(Marks*Weighing/100,2)),0) end Marks "&_
		"from ExamineItem EI "&_
		"left join (select ExaItemId,ExaEmpCode,isnull(convert(decimal(10,2),round(Weighing*Marks/100,2)),0) as Marks "&_
		"from ExaEmpTab) EET on EET.ExaItemId=EI.ExaItemId "&_
		"where EI.ExaItemId="&ExaItemId&" and EET.ExaEmpCode='"&EmpCode&"'"
	end if
	ExaFacSco.open StrSql,G_DBConn,1,1,1
...
Function AvgScore(ObjType,ObjCode,BOPId)
	...
	TotSco.open "select BOPId,avg(Marks) Marks from BegOfPerItem BI "&_
		...
		"sum(isnull(convert(decimal(10,2),round(ResultMarks,2)),0)) Marks "&_
		"from ExamineItem EI "&_
...
Function MaxId(ObjType,ObjCode,ExaPerId)
	...
	set rsLately=Server.CreateObject("ADODB.Recordset")
	...
	if isnull(MaxId) then
		rsLately.open "select Max(ExamineId) ExamineId from Examine E "&_
			"left join BegOfPerItem BI on BI.BOPItemId=E.BOPItemId "&_
			"left join BegOfPer BP on BP.BOPId=BI.BOPId "&_
			"where E.ExaObjType="&ObjType&" and ExaObjCode='"&ObjCode&"' "&_
			"and GradeState<>0 ",G_DBConn,1,1,1
		MaxId=rsLately("ExamineId")
		rsLately.close
	end if
	rsMaxId.close	
end Function
...
Function CopyFactor(CurExamineId,ExamineId)
	...
	set rsMarkItem=Server.CreateObject("ADODB.Recordset")
	...
	do while not rsCory.eof
		...
		rsEmpTab.close
		'复制评分办法及标准
		rsMark.open "select * from ExaMark where ExaItemId="&ExaItemId&"",G_DBConn,1,1,1
		do while not rsMark.eof
			rsMarkItem.open "select * from ExaMark ",G_DBConn,2,3,1
			rsMarkItem.addnew
				rsMarkItem("ExaItemId")=CurExaItemId
				rsMarkItem("ExaFacItemId")=rsMark("ExaFacItemId")
				rsMarkItem("MaxMarks")=rsMark("MaxMarks")
				rsMarkItem("MinMarks")=rsMark("MinMarks")
			rsMarkItem.update	
			rsMarkItem.close
			rsMark.movenext		
		loop
