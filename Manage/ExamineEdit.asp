...
<table border="0" align="center" cellpadding="0" cellspacing="0" width="800">
  <tr><td align="right"><font color="#FF0000"><%=strErr%></font>
    ...
    <input type="submit" name="Submits" value="重新设置评分">
...
<%
	...
	rsTemp.close
%>
		<option value="0" style="color:#0000FF">-----------------------</option>
<%

	rsTemp.open "select * from ExaFactor where ExaFactorId not in "&_
		"(select isnull(ExaFactorId,-1) from ExamineItem where ExaItemId<>"&ExaItemId&" and ExamineId="&ExamineId&" ) "&_
		"and IsDel=1 order by ExaFactorName ",G_DBConn,1,1,1
	do while not rsTemp.eof
	CurExaFactorId=rsTemp("ExaFactorId")
	ExaFactorName=rsTemp("ExaFactorName")
%>
		<option value="<%=CurExaFactorId%>" <%if trim(CurExaFactorId)=trim(ExaFactorId) then response.Write("selected") end if%> style="color:#999999"><%=ExaFactorName%></option>
<%
		rsTemp.movenext
	loop
	rsTemp.close
...
<%
	...
	do while not rsTemp.eof
		...
		SKName=rsTemp("SKName")
%>
	<option value="<%=ScoreKindId%>"><%=SKName%></option>
<%
		rsTemp.movenext
	loop
	rsTemp.close
